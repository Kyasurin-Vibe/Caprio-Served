  ...
  return <div className="bg-background px-8 pb-24 md:px-16 lg:px-20">
    ...
    <section id="bank-connection" className="mx-auto max-w-7xl border-t border-border py-20">
      ...
      <div className="mt-10 grid gap-2 sm:grid-cols-3" aria-label="Bank connection workflow">
        {[{ icon: Landmark, title: "Roe’s Kitchen sample", note: "D4 demo account" }, { icon: DatabaseZap, title: "Transaction search", note: "Scoped to the request" }, { icon: ShieldCheck, title: "Owner review", note: "Nothing sent automatically" }].map(({ icon: Icon, title, note }) => (
    ...
    <section id="record-matching" className="mx-auto grid max-w-7xl gap-8 border-t border-border py-20 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
      <div>
        ...
        <p className="type-body mt-5 max-w-xl text-base leading-7">On sample D4, Roe’s Kitchen business checking yields <strong className="font-medium text-foreground">7 include, 2 review, and 19 exclude</strong> from 28 transactions—John Doe payments only, not suppliers or other staff.</p>
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-primary p-6 text-white">
        ...
        <p className="type-caption mt-4 text-white/45">28 transactions searched · sample D4 · Use the D4 judge fixture in the app (not the BoA/Amex sandbox pickers).</p>
      </div>
    </section>
    ...
    <section id="resources" className="mx-auto grid max-w-7xl gap-10 border-b border-border py-20 md:grid-cols-2">
      <div>
        <h2 className="type-section">Richard runs a restaurant, not a legal department.</h2>
