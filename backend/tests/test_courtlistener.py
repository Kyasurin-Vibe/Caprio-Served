...
def test_federal_judge_suffix_does_not_change_the_case_number() -> None:
    assert normalize_case_number("5:25-cv-00000-AA-BB") == normalize_case_number(
        "5:25-cv-00000"
    )
    assert courtlistener_case_number("5:25-cv-00000-AA-BB") == "5:25-cv-00000"


def test_altered_federal_serial_number_stays_different() -> None:
    assert normalize_case_number("5:25-cv-90000-AA-BB") != normalize_case_number(
        "5:25-cv-00000"
    )


def test_party_match_requires_all_caption_sides_and_multiple_tokens() -> None:
    expected = ["John Doe", "Acme Consulting Services, Inc."]
    assert _party_overlap(expected, "Doe v. Acme Consulting Services", [])
    assert not _party_overlap(expected, "Doe v. Different Company", [])
    assert not _party_overlap(["John Doe"], "Doe v. Someone", [])
...
def test_near_match_is_close_sorted_and_in_claimed_court() -> None:
    parsed = DocumentParse(
        ...
        case_number="5:25-cv-90000-AA-BB",
        parties=["John Doe", "Acme Consulting Services, Inc."],
    )
    distance_two = _docket("5:25-cv-00108")
    d1_distance_one = _docket("5:25-cv-00000")
